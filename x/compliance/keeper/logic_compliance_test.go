...
var _ = Describe("logic_compliance_test.go", Ordered, func() {
	...
	It("Do not redistribute if no policy is set", func() {
		...
		Expect(s.GetCoinsFromModule(compliancetypes.MultiCoinRewardsRedistributionAccountName).String()).To(Equal("50acoin,70bcoin"))
	...
	It("Redistribute single coin, same weight", func() {
		...
		Expect(s.App().BankKeeper.GetAllBalances(s.Ctx(), validator1.AccAddress).String()).To(Equal("10000000000acoin,10000000000bcoin,10000000000ccoin,9200000000tkyve"))
		Expect(s.GetCoinsFromModule(compliancetypes.ModuleName).String()).To(BeEmpty())
		Expect(s.GetCoinsFromModule(compliancetypes.MultiCoinRewardsRedistributionAccountName).String()).To(Equal("70bcoin"))
		Expect(s.App().StakersKeeper.GetOutstandingRewards(s.Ctx(), validator1.Address, validator1.Address)).To(BeEmpty())
	...
	It("Redistribute multiple coins, same weight", func() {
		...
		Expect(s.App().BankKeeper.GetAllBalances(s.Ctx(), validator1.AccAddress).String()).To(Equal("10000000000acoin,10000000000bcoin,10000000000ccoin,9200000000tkyve"))
		Expect(s.GetCoinsFromModule(compliancetypes.ModuleName).String()).To(BeEmpty())
		Expect(s.GetCoinsFromModule(compliancetypes.MultiCoinRewardsRedistributionAccountName).String()).To(BeEmpty())
		Expect(s.App().StakersKeeper.GetOutstandingRewards(s.Ctx(), validator1.Address, validator1.Address)).To(BeEmpty())
	...
	It("Redistribute multiple coins, different weights", func() {
		...
		Expect(s.App().BankKeeper.GetAllBalances(s.Ctx(), validator1.AccAddress).String()).To(Equal("10000000000acoin,10000000000bcoin,10000000000ccoin,9200000000tkyve"))
		Expect(s.GetCoinsFromModule(compliancetypes.ModuleName).String()).To(BeEmpty())
		Expect(s.GetCoinsFromModule(compliancetypes.MultiCoinRewardsRedistributionAccountName).String()).To(Equal("1acoin")) // left over acoin
		Expect(s.App().StakersKeeper.GetOutstandingRewards(s.Ctx(), validator1.Address, validator1.Address)).To(BeEmpty())
