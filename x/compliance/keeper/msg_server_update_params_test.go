...
* Update refund pending time
* Update refund pending time with invalid value
...
var _ = Describe("msg_server_update_params.go", Ordered, func() {
	...
	It("Check default params", func() {
		...
		Expect(params.MultiCoinRefundPendingTime).To(Equal(types.DefaultMultiCoinRefundPendingTime))
		Expect(params.MultiCoinRefundPolicyAdminAddress).To(BeEmpty())
	})
	...
	It("Update every param at once", func() {
		// ARRANGE
		payload := `{
			"multi_coin_refund_pending_time": 10000,
		...
		// ASSERT
		updatedParams := s.App().ComplianceKeeper.GetParams(s.Ctx())

		Expect(submitErr).NotTo(HaveOccurred())
		Expect(voteErr).NotTo(HaveOccurred())

		Expect(updatedParams.MultiCoinRefundPendingTime).To(Equal(uint64(10000)))
		Expect(updatedParams.MultiCoinRefundPolicyAdminAddress).To(Equal("kyve1jq304cthpx0lwhpqzrdjrcza559ukyy3zsl2vd"))
	})
	...
	It("Update no params", func() {
		...
		Expect(updatedParams.MultiCoinRefundPendingTime).To(Equal(types.DefaultMultiCoinRefundPendingTime))
		Expect(updatedParams.MultiCoinRefundPolicyAdminAddress).To(BeEmpty())
	})

	It("Update with invalid formatted payload", func() {
		// ARRANGE
		payload := `{
			"multi_coin_refund_pending_time": "0.5",
		}`
	...
	It("Update refund pending time", func() {
		// ARRANGE
		payload := `{
			"multi_coin_refund_pending_time": 5
		}`
		...
		Expect(updatedParams.MultiCoinRefundPendingTime).To(Equal(uint64(5)))
		Expect(updatedParams.MultiCoinRefundPolicyAdminAddress).To(BeEmpty())
	})

	It("Update refund pending time with invalid value", func() {
		// ARRANGE
		payload := `{
			"multi_coin_refund_pending_time": "5.0"
		}`
	...
	It("Update policy admin address with invalid value", func() {
		// ARRANGE
		payload := `{
			"multi_coin_refund_policy_admin_address": "kyveabc"
		}`
