...
import { ADDRESS_ALICE, alice, bob, charlie } from "./helpers/accounts";
...
export const lcd = () => {
  ...
  test("Query <pools> and <pool> by id", async () => {
    ...
    //todo why status response is a string?
    poolsResponse.pools[0].status = 4;
    ...
    for (let pool of poolsResponse.pools) {
      ...
      if (poolsResponse.pool) {
        //todo why status response is a string?
        poolsResponse.pool.status = 4;
      }
      const vResult = validate(schema, poolsResponse);
      expect(vResult.valid).toBeTruthy();
    }
  ...
  test("Query <staker>", async () => {
    ...
    if (stakerResponse.staker) {
      stakerResponse.staker.status = 1;
    }
  ...
  // });
  //
  // test("Query <proposalByHeight>", async () => {
  //     const pool = await lcdClient.kyve.registry.v1beta1.pools({
  //         pagination: { limit: "1" },
  //     });
  ...
  test("Query <accountFundedList>", async () => {
    ...
    if (accountFundedListRes.funded[0].pool) {
      //todo
      accountFundedListRes.funded[0].pool.status = 1;
    }
  ...
  test("Query <accountStakedList>", async () => {
    ...
    if (accountStakedListRes.staked[0].pool) {
      accountStakedListRes.staked[0].pool.status = 1;
    }
  ...
  test("Query <delegatorsByPoolAndStaker>", async () => {
    ...
    if (delegatorsRes.pool) {
      delegatorsRes.pool.status = 1;
    }
  ...
  test("Query <stakersByPoolAndDelegator>", async () => {
    ...
    if (stakersByPoolAndDelegatorRes?.pool?.status) {
      stakersByPoolAndDelegatorRes.pool.status = 1;
    }
  ...
  test("Query <accountStakingUnbondingsRequest>", async () => {
    ...
    console.log(accountStakingUnbondingResponse);
